...
# Variables used are verbose and interface
verbose=""
interface=""

# Using while loop and case command to accept options for verbose mode and an interface name on the command line
# Setting the variable $verbose to contain string "yes" when user put -v option with command
while [ $# -gt 0 ]; do
  case "$1" in
    -v)
      verbose="yes"
      ...
      ;;
    *)
      # If the option used with the command on command line is the name of the interface
      # Giving the error message to the user if more than one interface name is used in the command
      # Double brackets are used to tackle with the complex expression
      if [[ -z "$interface" ]]; then
        interface="$1"
      else
        echo "OOPS! It looks like more than one interface name is used as argument."
        exit 1
      fi
      shift
      ;;
...
# Task 2
##############################################
# Getting the list of all interface names dynamically except for loopback interface
# awk is used to get the data which is required for the output
# grep is used to filter ip command output to get necessary data for variable $allinterfaces
if [[ -z $interface ]];then
  allinterfaces=$(ip -o link show | awk -F': ' '{print $2}' | grep -v "lo")
else
  # If no name provided by the user, getting list of all interfaces except loopback
  allinterfaces=$interface
fi

# Using for loop to generate report of every interface
for interface in $allinterfaces; do
    # Showing information of only that interface which has been specified by the user on command line
