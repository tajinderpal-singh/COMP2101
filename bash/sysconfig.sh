...
# The message is printed to standard error output by the use of >&2 in function
function error-message {
  echo "Sorry! An ERROR occured." >&2
}

# This function will send a message to stderr and exit with a failure status
# Usage:
#   error-exit ["some text to print to stderr" [exit-status]]
# The error message is redirected to standard error output by using >&2 in the funtion
function error-exit {
  error-message "$1" >&2
  exit 1
}
...
function displayhelp {
  ...
  echo "where  OPTIONS := "
  echo "  -h| --help:	Show help statements"
  echo "  --host:	Show hostname of the device"
  echo "  --domain:	Show the domain name"
  echo "  --ipconfig:	Show the network configuration data"
  echo "  --os:		Show the operating system information"
  echo "  --cpu:	Show the data about CPU"
  echo "  --memory:	Show the system memory information"
  echo "  --disk:	Show the disk usage information"
  echo "  --printer:	Show the list of printers"
}
...
# The function cleanup is used with INT signal that is produced when user presses Ctrl+C
trap cleanup INT
